...
const Home = React.forwardRef((props, ref) => {
  ...
  const easy = words.filter((word) => {
    return word.length >= 4 && word.length < 6;
  });
  const medium = words.filter((word) => {
    return word.length >= 7 && word.length < 9;
  });
  const hard = words.filter((word) => {
    return word.length >= 10 && word.length < 13;
  });
  const speedy = words.filter((word) => {
    return word.length >= 12 && word.length < 18;
  });
  ...
  useEffect(() => {
    if (start) {
      const timerID = setInterval(() => {
        setTimer((time) => (time !== 0 ? time - 1 : gg()));
      }, 1000);
      const gg = () => {
  ...
  const wordCheck = () => {
    if (difficulty === 1) {
      setWord(easy.sort(() => Math.random() - 0.5)[1]);
    }
    if (difficulty === 2) {
      setWord(medium.sort(() => Math.random() - 0.5)[1]);
    }
    if (difficulty === 3) {
      setWord(hard.sort(() => Math.random() - 0.5)[1]);
    }
    if (difficulty === 4) {
      setWord(speedy.sort(() => Math.random() - 0.5)[1]);
    }
  };
  ...
  const checkWord = (e) => {
    for (let i = 0; i < word.length; i++) {
      if (word === input) {
        correct.current.style.color = "green";
        setScore((score) => score + 10);
        setInput("");
        wordCheck();
      } else {
        correct.current.style.color = "red";
      }
    }
  ...
  const checkGameOver = () => {
    if (gameOver) {
      hide.current.classList.remove("hide");
    }
  };
  checkGameOver();
  const startGame = () => {
  ...
  return (
    <Box>
      ...
      {difficulty !== 0 && (
        <Box className="timer">
          {!gameOver && <h4>Countdown:{timer}s</h4>}
      ...
      <Box className="type-here">
        ...
        ></input>
        <Box>
          <button className="start" ref={hide} onClick={startGame}>
            Start
          </button>
          <button className="submit" onClick={checkWord}>
            Submit
          </button>
